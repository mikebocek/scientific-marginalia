...
Fortunately these people have already done a lot of the hard work for us. Since around the time of the French Revolution, scientists have largely agreed to all use the same set of units, which (owing to its origin) is refered to as the Système International (or SI) units. It's gone through some redefinition as our understanding of science has improved since the 18th century, but fundementally this is the system of units that scientists have used continuously for a little over 200 years. 
...
For the moment, we'll focus on the first three units, but in time we'll talk about all of these in detail except the candela. 

Unlike the other units, which express pretty universal things about nature, the candela is solidly grounded in how we (as humans) see brightness. It's roughly the total brightness of a single candle (hence the name), as radiated in all directions. In some ways this makes it one of the most directly relateable of the units, although most scientists (myself included) have never had any occasion to actually use it in our work.

Representing large and small numbers
------------------------------------

Science often deals with extremely large and small quantities. For example a DNA molecule is about 0.000000002 m wide, while the Moon is about 384400000 m away from the earth. These are not very convenient ways to write these numbers. Here's two better ways 
...
.. figure:: img/units/solargraph_example.jpg
   :width: 300
   :figwidth: 300
   :align: right
   :alt: Example view of a solargraph - a long exposure of the sun throughout the year 

   A long exposure view of the sun over the course of a year (a solargraph) taken in Budapest Hungary. Notice how the arc of the sun moves higher north in the sky as summer turns into winter (`from Wikipedia <https://en.wikipedia.org/wiki/Long-exposure_photography#/media/File:Solargraph_from_Sashegy_-_Budapest,_2014.01.01_-_2014.12.31_(1).jpg>`_)
